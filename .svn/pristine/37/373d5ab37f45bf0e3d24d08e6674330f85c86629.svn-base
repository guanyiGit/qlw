...
var popUpCal = {
    ...
    selectDate: function(Input) {
        ...
        //获取已选择时间
        var selectedYearMonthDay = this.formatDate(this.selectedDay, this.selectedMonth, this.selectedYear).replace("-", "/").replace("-", "/");
        var selectedHourMinuteSecond = $(Input).nextAll("input[type=text]").val() + ":00";
        //刷新分钟数
        if(undefined != selectedYearMonthDay && ":00" != selectedHourMinuteSecond){
            popUpCal.refreshMinutes(new Date(selectedYearMonthDay + " " + selectedHourMinuteSecond), 1);
        }

        //publish the message for once type callback
        if("once" == $(Input).attr("timetype")) resetoncetime();
        //publish the message for interval type callback
        if("interval" == $(Input).attr("timetype")) resetintervaltime();
    ...
    showCalendar: function() {
        ...
        var timeSelect = '';
        if (this.timeSeparators.length > 1) {
            timeSelect += '<div id="calendar_time"><div class="select" style="width:50px;margin-left:55px;"><select id="calendar_hour" style="width:50px">';
            if (this.timeSeparators.length == 4) {
    ...
    showFor: function(target) {
        ...
        //刷新分钟数
        if(undefined != $(target).prevAll("input[type=text]").val()){
            //获取当前已选择时间
            var selectedDate = new Date($(target).prevAll("input[type=text]").val().replace("-","/").replace("-","/") +
                " " + popUpCal.selectedHour + ":" + popUpCal.selectedMinute + ":" + "00");
            popUpCal.refreshMinutes(selectedDate, 2);
        }
    },
    ...
    showTime: function() {
        ...
        var timeSelect = '';
        if (this.timeSeparators.length > 1) {
            timeSelect += '<div id="calendar_time"><div class="select" style="width:50px;margin-left:25px;"><select id="calendar_hour" style="width:50px">';
            if (this.timeSeparators.length == 4) {
                if (this.selectedHour == 12 || this.selectedHour == 0) timeSelect += '<option value="12" selected="selected">12</option>';
                else timeSelect += '<option value="12">12</option>';
                for (var a = 1; a < 12; a++) {
                ...
                //检查当前小时设置最小值
                var minHour = this.minDate.getHours();
                for (var a = minHour; a < 24; a++) {
                    if (this.selectedHour == a) timeSelect += '<option value="' + a + '" selected="selected">' + a + '</option>';
                    else timeSelect += '<option value="' + a + '">' + a + '</option>';
                }
            }
            ...
            timeSelect += '</select></div> <div class="select" style=" width:10px;">' + this.timeSeparators[1] + '</div> <div class="select" style="width:37px"><select id="calendar_minute" style="width:40px">';
            //检查当前分钟设置最小值
            var minMinutes = this.minDate.getMinutes();
            for (var a = minMinutes; a <= 59; a++) {
    ...
    timeActions: function() {
        if (this.timeSeparators.length > 1) {
            $('#calendar_hour').change(function() {	//change hour
                ...
                //publish the message for once type callback
                if("once" == $(popUpCal.input).attr("timetype")) resetoncetime();
                //publish the message for interval type callback
                if("interval" == $(popUpCal.input).attr("timetype")) resetintervaltime();

                //获取当前已选择时间
                if(undefined != $(popUpCal.input).prevAll("input[type=text]")){
                    var selectedDate = new Date($(popUpCal.input).prevAll("input[type=text]").val().replace("-","/").replace("-","/") +
                        " " + popUpCal.selectedHour + ":" + popUpCal.selectedMinute + ":" + "00");
                    popUpCal.refreshMinutes(selectedDate, 2);
                }
            });
    ...
    formatTime: function(hour, minute) {
        var timeString = '';
        var customTimeSeparators = [' ', ':'];
        //append time info
        if (customTimeSeparators && customTimeSeparators.length > 1 && customTimeSeparators[0] && hour != null && minute != null) {
            //alert(timeString)
            timeString += customTimeSeparators[0] + this.hourString(hour) + customTimeSeparators[1] + ((minute >= 10) ? minute : ('0' + minute));
            if (customTimeSeparators.length == 4 && hour < 12) timeString += customTimeSeparators[2];
            else if (customTimeSeparators.length == 4 && hour >= 12) timeString += customTimeSeparators[3];
        }
        return timeString.substring(1);
    },
    /**
     * 刷新分钟数
     * @param chooseTime    已选择时间(DATE类型)
     * @param inputType     输入框类型(1:年月日,2:时分秒)
     */
    refreshMinutes: function(chooseTime, inputType){
        /*********改变分钟可选项*********/
            //最小时间加一天
        var minDateUp = new Date(popUpCal.minDate);
            minDateUp.setDate(minDateUp.getDate() + 1);
        //设定最小小时数0(0为不限制)
        var minHour = 0;
        //设定最小分钟数0(0为不限制)
        var minMinutes = 0;
        //若当前时间小于最小时间则应对分钟数做控制
        if(chooseTime.getTime() < minDateUp.getTime()){
            //检查当前小时设置最小值
            minHour = popUpCal.minDate.getHours();
            //检查当前分钟设置最小值
            minMinutes = popUpCal.minDate.getMinutes();
            if(undefined != popUpCal.input){
                //若上一个输入框未找到，则当前输入框为年月日框
                if(0 == $(popUpCal.input).prevAll("input[type=text]").length){
                    $(popUpCal.input).nextAll("input[type=text]").val(popUpCal.formatTime(popUpCal.selectedHour, minMinutes))
                }else{
                    //重新加载时间
                    popUpCal.input.val(popUpCal.formatTime(popUpCal.selectedHour, minMinutes));
                }
        ...
        //若日期不变，则小时数不刷新
        if(popUpCal.minDate.getDay() + 1 == chooseTime.getDay()){
            minHour = popUpCal.minDate.getHours();
        }
        ...
        if(1 == inputType){
            ...
            for(var i = minHour; i < 24; i++){
                var $option = $("<option>").attr("value", i).text(i < 10 ? "0" + i : i);
                //默认选中
                if(i == popUpCal.selectedHour) $option.attr("selected", "selected");
                $("#calendar_hour").append($option);
                //若时间为初始值且为最后一次循环则重新加载时间
                if(0 == minHour && 23 == i){
                    //若上一个输入框未找到，则当前输入框为年月日框
                    if(0 == $(popUpCal.input).prevAll("input[type=text]").length){
                        $(popUpCal.input).nextAll("input[type=text]").val(popUpCal.formatTime(popUpCal.selectedHour, popUpCal.selectedMinute))
                    }else{
                        //重新加载时间
                        popUpCal.input.val(popUpCal.formatTime(popUpCal.selectedHour, popUpCal.selectedMinute));
                    }
            ...
            //清空分钟数
            $("#calendar_minute").html("");
            //重新加载分钟数
            for(var i = minMinutes; i < 60; i++){
                var $option = $("<option>").attr("value", i).text(i < 10 ? "0" + i : i);
                //默认选中
                if(i == popUpCal.selectedMinute) $option.attr("selected", "selected");
                $("#calendar_minute").append($option);
                //若时间为初始值且为最后一次循环则重新加载时间
                if(0 == minMinutes && 59 == i){
                    //若上一个输入框未找到，则当前输入框为年月日框
                    if(0 == $(popUpCal.input).prevAll("input[type=text]").length){
                        $(popUpCal.input).nextAll("input[type=text]").val(popUpCal.formatTime(popUpCal.selectedHour, popUpCal.selectedMinute))
                    }else{
