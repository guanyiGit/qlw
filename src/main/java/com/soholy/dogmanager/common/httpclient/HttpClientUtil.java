package com.soholy.dogmanager.common.httpclient;
...
public class HttpClientUtil {
    ...
    private static void setHttpClientConnectionPool(PoolingHttpClientConnectionManager cm) {
        if (cm != null) {
            ...
            cm.setDefaultMaxPerRoute(5);// 每路由最大连接数，默认是2
        }
    }
     ...
     *
     * @param httpClient
     *            HttpClient客户端实例，传入null会自动创建一个HttpClient
     * @param url
     *            请求的远程地经
     * @param charset
     *            请求编码，默认UTF8
     * @param closeHttpClient
     *            执行请求结束后是否关闭HttpClient客户端实
     * @param headers
    ...
    public static HttpResult executeBaseHttp(String url, String methodName) throws ParseException, IOException, URISyntaxException{
    	return executeBaseHttp(url, methodName, true, DEFAULT_CHARSET);
    }
    
    public static HttpResult executeBaseHttp(String url, String methodName, Boolean isCloseHttpClien) throws ParseException, IOException, URISyntaxException{
    	return executeBaseHttp(url, methodName, isCloseHttpClien, DEFAULT_CHARSET);
    }
    /**
     * @Description (执行http请求)
     * @param url
    ...
    public static HttpResult executeHttpParams(String url, String methodName, 
            Object paramsObj) throws ParseException, IOException, URISyntaxException{
    	return executeHttpParams(url, methodName, true, null, paramsObj, DEFAULT_CHARSET);
    }
    
    public static HttpResult executeHttpParams(String url, String methodName, 
            Map<String, String> headers, Object paramsObj) throws ParseException, IOException, URISyntaxException{
    	return executeHttpParams(url, methodName, true, headers, paramsObj, DEFAULT_CHARSET);
    }
    
    public static HttpResult executeHttpParams(String url, String methodName, 
            Map<String, String> headers, Object paramsObj, String charset) throws ParseException, IOException, URISyntaxException{
    	return executeHttpParams(url, methodName, true, headers, paramsObj, charset);
    }
    ...
    public static CloseableHttpClient createHttpClient(int retryTimes, int socketTimeout) {
        ...
        if (socketTimeout >= 0) {
            builder.setSocketTimeout(socketTimeout);// 请求获取数据的超时时间，单位毫秒 如果访问多个接口，多少时间内无法返回数据，就直接放弃此次调用
        }
    ...
    private static void enableSSL() {
        try {
            ...
            socketFactory = new SSLConnectionSocketFactory(ssl, NoopHostnameVerifier.INSTANCE);//设置域名不检查
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static HttpResult getResult(CloseableHttpResponse httpResponse, String charset)
        ...
        EntityUtils.consume(entity);// 关闭应该关闭的资源，适当的释放资;也可以把底层的流给关闭了
        return httpResult;
    }
