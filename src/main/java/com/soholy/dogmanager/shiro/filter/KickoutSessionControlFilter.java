package com.soholy.dogmanager.shiro.filter;
...
public class KickoutSessionControlFilter extends AccessControlFilter {
	...
	public static final String DEFAULT_KICKOUT_CACHE_KEY_PREFIX = "kickout:";
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		...
		// 读取缓存 没有就存入
		Deque<Serializable> deque = cache.get(getRedisKickoutKey(username));
		...
		if (!deque.contains(sessionId) && session.getAttribute("kickout") == null) {
			...
			// 将用户的sessionId队列缓存
			cache.put(getRedisKickoutKey(username), deque);
		}
